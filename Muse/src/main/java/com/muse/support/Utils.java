...
public class Utils {
	...
	public static int RANDOM=getRandomCount();
	/**
	 * 是否开启debug，
	 */
				...
				if (SPECIAL_ENTITIY.contains(entry.getKey())) {// 判断是否在之中
					...
					if (ENTITY_STRING.equals(entry.getKey())) {// string
						...
						for (File file : files) {
							...
							// builder.addPart(String.valueOf(map.get(ENTITY_MULTIPART+".name")) + count++,
							// fileBody);
	...
	public static void main(String[] args) {
		for (int i = 0; i < 10; i++) {
			System.out.println(getRandomCount());
		}
	}
}
