...
public class HttpClientUtil{
	...
	static{
		try {
			client4HTTP = HttpClientPool.custom().build();
			client4HTTPS = HttpClientPool.custom().ssl().build();
	...
	private static void create(HttpConfig config) throws HttpProcessException  {
		if(config.httpClientPool()!=null && config.httpClientPool().isSetPool){ //如果设置了hcb对象，且配置了连接池，则直接从连接池取
			if(config.url().toLowerCase().startsWith("https://")){
				config.client(config.httpClientPool().ssl().build());
			}else{
				config.client(config.httpClientPool().build());
			}
		}else{
